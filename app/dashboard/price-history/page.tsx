...
const TABLE_PAGE_SIZE = 10; // rows per vendor per page
...
function calcDomain(vendors: VendorHistory[]): [number, number] {
  const all = vendors.flatMap(v => v.priceHistory.map(p => p.price)).filter(p => p > 0);
...
function VendorTableSection({ vendor, colorIdx }: { vendor: VendorHistory; colorIdx: number }) {
  const [page, setPage] = useState(0);
  const color   = COLORS[colorIdx % COLORS.length];
  const rows    = [...vendor.priceHistory].reverse(); // newest first
  ...
  return (
    <>
      ...
      {/* Data rows */}
      {slice.map((pt, pi) => {
        const prev = rows[page * TABLE_PAGE_SIZE + pi + 1]?.price ?? null;
        ...
        <tr className="border-t border-border/40 bg-muted/20">
          <td colSpan={5} className="px-4 py-2">
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              <span>
                Showing {page * TABLE_PAGE_SIZE + 1}–{Math.min((page + 1) * TABLE_PAGE_SIZE, total)} of {total}
              </span>
...
function ProductCard({ product }: { product: ProductData }) {
  ...
  const selData   = selVendor
    ? selVendor.priceHistory.map(p => ({ date: p.date, price: p.price }))
    : [];
    ...
    <div className="bg-card border border-border rounded-xl overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-border">
        ...
        <div className="flex flex-wrap gap-4 mt-1 text-xs text-muted-foreground">
          ...
          <span className="font-medium text-foreground">{product.vendors.length} vendor{product.vendors.length !== 1 ? 's' : ''}</span>
        </div>
      </div>
      ...
      <div className="border-t border-border">
        ...
        <button
          ...
          <div className="flex items-center gap-2 text-muted-foreground text-xs font-normal">
            <span>
              {product.vendors.reduce((n, v) => n + v.priceHistory.length, 0)} total records
            </span>
...
export default function PriceHistoryPage() {
  ...
  useEffect(() => {
    try {
      sessionStorage.setItem(SESSION_KEY, JSON.stringify({ query, from, to, products }));
    } catch { /* quota exceeded — ignore */ }
  }, [query, from, to, products]);

  const search = useCallback(async (overrideQuery?: string, overrideFrom?: string, overrideTo?: string) => {
    ...
    <div className="p-6 space-y-6 min-h-full bg-background">
      ...
      {searched && !loading && products.length > 0 && (
        <p className="text-sm text-muted-foreground">
          Found <span className="text-foreground font-semibold">{products.length}</span> product{products.length !== 1 ? 's' : ''} matching <span className="text-foreground font-semibold">"{query}"</span>
        </p>
      )}
      ...
      {products.map(product => (
        <ProductCard key={product.upc || product.productCode} product={product} />
      ))}
    </div>
  );
}
